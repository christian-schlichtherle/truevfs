...
private abstract class AspectController(controller: FsController)
  ...
  private class Input(socket: InputSocket[_ <: Entry]) extends AbstractInputSocket[Entry] {
    ...
    override def stream(peer: OutputSocket[_ <: Entry]): InputStream =
      apply(() => socket stream peer)

    override def channel(peer: OutputSocket[_ <: Entry]): SeekableByteChannel =
  ...
  private class Output(socket: OutputSocket[_ <: Entry]) extends AbstractOutputSocket[Entry] {
    ...
    override def stream(peer: InputSocket[_ <: Entry]): OutputStream =
      apply(() => socket stream peer)

    override def channel(peer: InputSocket[_ <: Entry]): SeekableByteChannel =
      apply(() => socket channel peer)
  }
