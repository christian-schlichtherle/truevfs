...
final class FileSystem[K >: Null, V](
  ...
  override def +=(kv: (K, V)): this.type = { link(kv._1, kv._2); this }
  ...
  override def -=(path: K): this.type = { unlink(path); this }
  ...
  override def stringPrefix: String = "FileSystem"
}
...
object FileSystem {
  ...
  sealed abstract class Node[K >: Null, +V] extends Iterable[(K, Node[K, V])] {
    ...
    final override def stringPrefix: String = "Node"

    final override def toString(): String = {
      stringPrefix + "(path=" + path + ", isLeaf=" + isLeaf + ", entry=" + entry + ")"
    }
  }
