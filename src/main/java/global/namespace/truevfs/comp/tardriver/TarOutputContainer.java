 ...
 * @see TarInputContainer
 */
@SuppressWarnings("OptionalUsedAsFieldOrParameterType")
public final class TarOutputContainer implements OutputContainer<TarDriverEntry> {
    ...
    public TarOutputContainer(final FsModel model, final Sink sink, final TarDriver driver) throws IOException {
    ...
    public OutputSocket<TarDriverEntry> output(final TarDriverEntry local) {
        ...
        class Output implements OutputSocket<TarDriverEntry> {

            @Override
            public TarDriverEntry getTarget() {
                return local;
            }
            ...
            public OutputStream stream(final Optional<? extends InputSocket<? extends Entry>> peer) throws IOException {
                ...
                updateProperties(local, peer.isPresent() ? Optional.of(peer.get().getTarget()) : Optional.empty());
    ...
    private final class BufferedEntryOutputStream extends DecoratingOutputStream {
        ...
        void storeBuffer() throws IOException {
            ...
            try (final InputStream in = buffer.input().stream(Optional.empty())) {
                final TarArchiveOutputStream taos = TarOutputContainer.this.taos;
                taos.putArchiveEntry(local);
                Streams.cat(in, taos);
                taos.closeArchiveEntry();
