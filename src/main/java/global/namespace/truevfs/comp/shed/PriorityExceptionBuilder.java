...
public class PriorityExceptionBuilder<X extends Throwable> extends AbstractExceptionBuilder<X, X> {
     ...
     * assembly.
     *
     * @param comparator the comparator used for prioritizing the exceptions in the assembly.
     */
    ...
    @Override
    protected final X update(final X input, final Optional<X> assembly) {
        exceptions.add(input);
        if (assembly.isPresent()) {
            final X t = assembly.get();
