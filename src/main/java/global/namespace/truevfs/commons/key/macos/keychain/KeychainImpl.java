...
import global.namespace.truevfs.commons.key.macos.keychain.Security.*;
import lombok.val;
...
import static global.namespace.truevfs.commons.key.macos.keychain.KeychainUtils.list;
import static global.namespace.truevfs.commons.key.macos.keychain.KeychainUtils.map;
import static global.namespace.truevfs.commons.key.macos.keychain.Security.*;
import static global.namespace.truevfs.commons.shed.Buffers.byteBuffer;

/**
 * The default implementation of {@link Keychain}.
 * Unfortunately, Apple's Keychain Services API isn't thread-safe, so this implementation provides a rather
 * coarse-grained approach to achieve thread-safety.
...
final class KeychainImpl extends Keychain {
    ...
    KeychainImpl(final String path, final Supplier<char[]> password) throws KeychainException {
        val chars = password.get();
        try {
            val prompt = 0 >= chars.length;
            val pr = new PointerByReference();
            val buffer = byteBuffer(chars);
    ...
    public synchronized void visitItems(
        ...
        {
            val srr = new PointerByReference();
        ...
        class Visit implements Callable<Void> {
            ...
            private final EnumMap<ItemClass, SecKeychainAttributeInfo> infos = new EnumMap<>(ItemClass.class);
            ...
            public Void call() throws KeychainException {
                while (true) {
                    ...
                    {
                        val irr = new PointerByReference();
                        val status = SecKeychainSearchCopyNext(sr, irr);
                        switch (status) {
                    ...
                    class ItemImpl implements Item {
                        ...
                        @Override
                        public @Nullable
                        ByteBuffer getAttribute(AttributeClass id) throws KeychainException {
                        ...
                        public void setAttribute(
                            ...
                            val attributes = new EnumMap<AttributeClass, ByteBuffer>(AttributeClass.class);
                        ...
                        private Map<AttributeClass, ByteBuffer> getAttributes(
                            ...
                            val ar = new PointerByReference();
                            check(SecKeychainItemCopyAttributesAndData(
                                    ir, info, null, ar, null, null));
                            val l = new SecKeychainAttributeList(ar.getValue());
                            try {
                                l.read();
                                return map(l);
                            } finally {
                        ...
                        public void putAttributeMap(
                            ...
                            check(SecKeychainItemModifyAttributesAndData(ir, list(attributes), 0, null));
                        }

                        @Override
                        public ByteBuffer getSecret() throws KeychainException {
                            val lr = new IntByReference();
                            val dr = new PointerByReference();
                            check(SecKeychainItemCopyAttributesAndData(
                                    ir, null, null, null, lr, dr));
                            val p = dr.getValue();
                            try {
                                val l = lr.getValue() & 0xFFFF_FFFFL;
                        ...
                        public void setSecret(ByteBuffer secret) throws KeychainException {
                            check(SecKeychainItemModifyAttributesAndData(ir, null, secret.remaining(), secret));
                        }
    ...
    private static void check(final int status) throws KeychainException {
