...
public interface Key<K extends Key<K>> extends Cloneable {
    ...
    /**
     * Returns {@code true} if and only if this safe key deeply equals the given object.
     * A safe key equals another object if and only if the other object has the same runtime class and all its
     * properties compare deeply equal.
     */
    @Override
     ...
     * This method is provided for completeness only - you should actually never use secret keys as hash map keys
     * because of their mutable properties!
     */
    @Override
    int hashCode();
}
