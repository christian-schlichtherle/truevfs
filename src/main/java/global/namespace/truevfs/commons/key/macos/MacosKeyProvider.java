...
/**
 * Interacts with the {@link MacosKeyManager} to persist passwords into Apple's Keychain Services API.
 *
 * @author Christian Schlichtherle
 */
final class MacosKeyProvider<P extends AbstractPromptingPbeParameters<P, ?>> implements KeyProvider<P> {

    private final MacosKeyManager<P> manager;
    private final URI resource;
    private final KeyProvider<P> provider;
    private volatile Optional<P> param = Optional.empty();

    MacosKeyProvider(
            final MacosKeyManager<P> manager,
            final URI resource,
