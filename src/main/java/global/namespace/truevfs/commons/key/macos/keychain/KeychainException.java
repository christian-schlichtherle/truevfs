...
import static global.namespace.truevfs.commons.key.macos.keychain.Security.errSecDuplicateItem;
import static global.namespace.truevfs.commons.key.macos.keychain.Security.message;
...
public class KeychainException extends Exception {
    ...
    static KeychainException create(final int status) {
        return status == errSecDuplicateItem
                ? new DuplicateItemException()
                : new KeychainException(status);
    }
    ...
    public String getMessage() {
        return String.format(Locale.ENGLISH, "%s (%d)", message(status), status);
    }
}
