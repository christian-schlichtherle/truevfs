...
import global.namespace.truevfs.commons.zip.AbstractZipOutputStream;
 ...
 * @see ZipInputContainer
 */
@SuppressWarnings("OptionalUsedAsFieldOrParameterType")
public final class ZipOutputContainer<E extends AbstractZipDriverEntry>
        extends AbstractZipOutputStream<E> implements OutputContainer<E> {

    private final FsModel model;
    private final AbstractZipDriver<E> driver;
    private @CheckForNull
    IoBuffer postamble;
    private Optional<E> bufferedEntry = Optional.empty();
    private @CheckForNull
    ZipCryptoParameters param;

    public ZipOutputContainer(
            final FsModel model,
            final FsOutputSocketSink sink,
            final @CheckForNull ZipInputContainer<E> source,
            ...
            if (null != source) {
                ...
                if (0 < source.getPostambleLength()) {
                    ...
                    Streams.copy(source.getPostambleInputStream(),
                            this.postamble.output().stream(Optional.empty()));
                }
            }
    ...
    public Collection<E> entries() {
        return new AbstractCollection<E>() {
            ...
            public Iterator<E> iterator() {
                return ZipOutputContainer.this.iterator();
            }

            @Override
            public int size() {
                return ZipOutputContainer.this.size();
            }
        };
    }
    ...
    public Iterator<E> iterator() {
        return bufferedEntry
                .<Iterator<E>>map(e -> new CompoundIterator<>(super.iterator(), Collections.singletonList(e).iterator()))
                .orElseGet(super::iterator);
    }

    @Override
    public int size() {
        return super.size() + (bufferedEntry.isPresent() ? 1 : 0);
    }

    @Override
    public Optional<E> entry(final String name) {
        Optional<E> entry = super.entry(name);
        if (entry.isPresent()) {
            return entry;
        } else {
            entry = this.bufferedEntry;
            return entry.map(AbstractZipDriverEntry::getName).filter(name::equals).isPresent()
                    ? entry
                    : Optional.empty();
        }
    }
    ...
    public OutputSocket<E> output(final E local) { // local target
        ...
        class Output implements OutputSocket<E> {
            ...
            public OutputStream stream(final Optional<? extends InputSocket<? extends Entry>> peer) throws IOException {
                ...
                final boolean rdc = updateProperties(local,
                        peer.isPresent() ? Optional.of(peer.get().getTarget()) : Optional.empty());
    ...
    public boolean isBusy() {
        return super.isBusy() || bufferedEntry.isPresent();
    }
    ...
    public void close() throws IOException {
        ...
        if (null != postamble) {
            ...
            Throwable ex = null;
            try {
                try (final InputStream in = input.stream(Optional.empty())) {
                    ...
                    final long ipl = input.getTarget().getSize(DATA);
                    if ((ol + ipl) % 4 != 0)
    ...
    private final class BufferedEntryOutputStream
        ...
        BufferedEntryOutputStream(final E local)
            ...
            bufferedEntry = Optional.of(local);
        }
        ...
        void updateProperties() {
            ...
            ZipOutputContainer.this.updateProperties(local, Optional.of(buffer));
        }

        void storeBuffer() throws IOException {
            final IoBuffer buffer = this.buffer;
            Throwable t1 = null;
            try (final InputStream in = buffer.input().stream(Optional.empty())) {
                final ZipOutputContainer<E> zos = ZipOutputContainer.this;
