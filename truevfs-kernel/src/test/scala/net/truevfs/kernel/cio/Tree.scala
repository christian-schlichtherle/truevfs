...
/**
 * A container which persists entries in a tree.
 ...
 */
class Tree[E <: AnyRef](splitter: Splitter = new Splitter('/'))
extends Iterable[E] {
  ...
  override def iterator = rootNode recursiveIterator
} // Tree

object Tree {

  private final class Node[E <: AnyRef]
  (private[this] var _entry: Option[E])
  (implicit container: Tree[E])
  extends collection.Iterable[Node[E]] {
    ...
    def entry_=(entry: Option[E])(implicit container: Tree[E]) {
      // HC SVNT DRACONES!
      if (_entry isDefined) {
        if (entry isEmpty)
          container._size -= 1
