...
/**
 * A {@link TreeModel} which traverses {@link File} instances.
 * If the root of this tree model is actually an instance of {@link TFile},
 ...
 * @version $Id$
 */
public class TFileTreeModel implements TreeModel {
     ...
     * Maps {@link File} -&gt; {@link File}[] instances.
     */
    // Tactical note: Working with a WeakHashMap shows strange results.
    private final Map<File, File[]> cache
    ...
    /** A comparator for {@code File} or super classes. */
    private final Comparator<? super File> comparator;

    private final EventListenerList listeners = new EventListenerList();

    /**
     * Equivalent to {@link #TFileTreeModel(File, FileFilter, Comparator)
     * TFileTreeModel(null, null, FILE_NAME_COMPARATOR)}.
    ...
    /**
     * Equivalent to {@link #TFileTreeModel(File, FileFilter, Comparator)
     * TFileTreeModel(root, null, FILE_NAME_COMPARATOR)}.
     */
    public TFileTreeModel(final File root) {
        this(root, null, FILE_NAME_COMPARATOR);
    }

    /**
     * Equivalent to {@link #TFileTreeModel(File, FileFilter, Comparator)
     * TFileTreeModel(root, filter, FILE_NAME_COMPARATOR)}.
     */
    public TFileTreeModel(
            final File root,
     ...
     * @param comparator A comparator for {@code File} instances
     *        or super classes.
     ...
     *         {@code Comparator} for {@code File} or super
     *         class instances.
     */
    public TFileTreeModel(
            final File root,
            final FileFilter filter,
            final Comparator<? super File> comparator) {
        if (comparator == null)
     ...
     * This is actually an instance of {@link File File} or
     * a subclass, like {@link TFile}.
     *
     * @return A {@code TFile} object or {@code null} if this tree
     *         model has not been created with a {@code TFile} object.
     */
    @Override
	...
	public Object getChild(Object parent, int index) {
        final File[] children = getChildren((File) parent);
    ...
    private File[] getChildren(final File parent) {
        assert parent != null;
        File[] children = cache.get(parent);
        if (children == null) {
     ...
     * is {@code null}.
     */
    public TreePath newTreePath(final File node) {
        File[] elements = newPath(node);
        return elements != null ? new TreePath(elements) : null;
    }

    /**
     * Returns an array of {@link File} objects indicating the path
     ...
     *         {@code null}.
     */
    private File[] newPath(final File node) {
    ...
    private File[] newPath(final File node, int level) {
     ...
     */
    public boolean createNewFile(final File node)
    throws IOException {
     ...
     * @throws IOException If an I/O error occurs.
     */
    public boolean mkdir(final File node) {
        if (!node.mkdir())
            return false;
        nodeInserted(node);
        return true;
     ...
     * @throws IOException If an I/O error occurs.
     */
    public boolean mkdirs(final File node) {
        if (!node.mkdirs())
            return false;
        nodeInserted(node);
        return true;
     ...
     */
    public boolean copyTo(final TFile oldNode, final File node) {
     ...
     * @return Whether or not the file has been successfully renamed.
     */
    public boolean copyAllTo(final TFile oldNode, final File node) {
     ...
     * @return Whether or not the file has been successfully renamed.
     */
    public boolean archiveCopyTo(final TFile oldNode, final File node) {
     ...
     * @return Whether or not the file has been successfully renamed.
     */
    public boolean archiveCopyAllTo(final TFile oldNode, final File node) {
     ...
     * @throws IOException If an I/O error occurs.
     */
    public boolean delete(final File node) {
        if (!node.delete())
            return false;
        nodeRemoved(node);
        return true;
     ...
     */
    public void nodeInsertedOrStructureChanged(final File node) {
        if (node == null)
            throw new NullPointerException();
     ...
     * on all listeners of this {@code TreeModel}.
     */
    public void nodeInserted(final File node) {
        if (cache.containsKey(node))
            return;
        final File parent = node.getParentFile();
        forget(parent, false);
     ...
     * on all listeners of this {@code TreeModel}.
     */
    public void nodeChanged(final File node) {
        final File parent = node.getParentFile();
     ...
     * on all listeners of this {@code TreeModel}.
     */
    public void nodeRemoved(final File node) {
        final File parent = node.getParentFile();
        final int index = getIndexOfChild(parent, node); // old index
        if (index == -1)
            return;
    ...
    /** Alias for {@link #structureChanged(File)}. */
    public final void refresh(final File node) {
        structureChanged(node);
    }
     ...
     * on all listeners of this {@code TreeModel}.
     */
    public void structureChanged(final File node) {
        if (node == null)
            throw new NullPointerException();
        forget(node, true);
        fireTreeStructureChanged(
    ...
    private void forget(
        ...
        final File[] children = cache.remove(node);
