 ...
 */
@ThreadSafe
class FSStatisticsController1
extends FsDecoratorController<FSModel1, FsController<?>> {

    private final FSStatisticsManager1 manager;
     ...
     */
    FSStatisticsController1(
            @NonNull FsController<?> controller,
            @NonNull FSStatisticsManager1 manager) {
        super(controller);
        assert null != manager;
    ...
    private class Input extends DecoratorInputSocket<Entry> {
        Input(FSEntryName1 name, BitField<FSInputOption1> options) {
    ...
    private class Output extends DecoratorOutputSocket<Entry> {
        Output(FSEntryName1 name, BitField<FSOutputOption1> options, Entry template) {
