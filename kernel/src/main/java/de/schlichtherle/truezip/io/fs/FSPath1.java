 ...
 *     is parsed according to the syntax constraints for an {@link FSEntryName1}
...
public final class FSPath1 implements Serializable, Comparable<FSPath1> {
     ...
     * {@link FSMountPoint1 mount points} and {@link EntryName entry names}.
     */
    public static final String MOUNT_POINT_SEPARATOR = "!" + SEPARATOR;

    private @NonNull URI uri; // not final for serialization only!

    private transient @Nullable FSMountPoint1 mountPoint;

    private transient @NonNull FSEntryName1 entryName;

    private transient volatile @Nullable FSPath1 hierarchical;

    /**
     * Equivalent to {@link #create(String, boolean) create(uri, false)}.
     */
    public static @NonNull FSPath1
     ...
     * {@link #FSPath1(String, boolean) new FSPath1(uri, normalize)}
    ...
    create(@NonNull String uri, @NonNull FsUriModifier modifier) {
        try {
            return new FSPath1(uri, modifier);
     ...
     * This static factory method calls
     * {@link #FSPath1(URI, boolean) new FSPath1(uri, normalize)}
     * and wraps any thrown {@link URISyntaxException} in an
     * {@link IllegalArgumentException}.
     *
    ...
    /**
     * Equivalent to {@link #FSPath1(String, boolean) new FSPath1(uri, false)}.
     */
    public FSPath1(@NonNull String uri) throws URISyntaxException {
     ...
     *         syntax constraints for paths.
     */
    public FSPath1(@NonNull String uri, @NonNull FsUriModifier modifier) throws URISyntaxException {
        parse(uri, modifier);
    }

    /**
     * Equivalent to {@link #FSPath1(URI, boolean) new FSPath1(uri, false)}.
     */
    public FSPath1(@NonNull URI uri) throws URISyntaxException {
        parse(uri, FsUriModifier.NONE);
    }
     ...
     *         syntax constraints for paths.
     */
    public FSPath1(@NonNull URI uri, @NonNull FsUriModifier modifier)
     ...
     */
    public FSPath1(  final @CheckForNull FSMountPoint1 mountPoint,
                    final @NonNull FSEntryName1 entryName) {
        if (null == mountPoint) {
    ...
    throws URISyntaxException {
        ...
        if (uri.isOpaque()) {
            ...
            mountPoint = new FSMountPoint1(
                    new URI(uri.getScheme(), ssp.substring(0, i + 2), null),
                    modifier);
            entryName = new FSEntryName1(
            ...
            mountPoint = new FSMountPoint1(uri.resolve("."), FsUriModifier.NONE);
            entryName = new FSEntryName1(mountPoint.getUri().relativize(uri), FsUriModifier.NONE);
        } else {
            mountPoint = null;
            entryName = new FSEntryName1(uri, modifier);
     ...
     */
    public @Nullable FSMountPoint1 getMountPoint() {
        return mountPoint;
    }
     ...
     */
    public @NonNull FSEntryName1 getEntryName() {
        return entryName;
    }
     ...
     */
    public @NonNull FSPath1
    resolve(final @NonNull FSEntryName1 entryName) {
        return new FSPath1(
                this.mountPoint,
                new FSEntryName1(this.entryName, entryName));
    }

    /**
     * Returns a hierarchical URI for this path.
     * If this path's {@link #getUri() URI} is opaque, the
     * {@link FSMountPoint1#hierarchicalize() hierarchical URI} of its
     ...
     */
    public @NonNull FSPath1 hierarchicalize() {
        return null != hierarchical
                ? hierarchical
                : (hierarchical = !uri.isOpaque()
                    ? this
                    : 0 == entryName.toString().length()
                        ? FSPath1.create(mountPoint.hierarchicalize().getUri())
    ...
    public boolean equals(@CheckForNull Object that) {
        return this == that
                || that instanceof FSPath1
                    && this.uri.equals(((FSPath1) that).uri);
    }
     ...
     */
    @Override
    public int compareTo(@NonNull FSPath1 that) {
