 ...
 * {@link ArchiveControllers.ShutdownHook#SINGLETON} when the JVM terminates.
 *
 * @see ArchiveControllers#addToShutdownHook(java.lang.Runnable)
 */
final class JVMShutdownHook extends Thread {
    ...
    private JVMShutdownHook() {
        super(  ArchiveControllers.ShutdownHook.SINGLETON,
     ...
     * {@link ArchiveControllers.ShutdownHook#SINGLETON} when the JVM
     * terminates.
     */
    public synchronized void add(final Runnable runnable) {
        ArchiveControllers.ShutdownHook.SINGLETON.add(runnable);
    }
}
